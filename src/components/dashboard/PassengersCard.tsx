...
import { ConnectingPassengersPanel } from "./ConnectingPassengersCard";
...
export function PassengersCard({
  ...
  return (
    ...
    >
      <PillTabs
        tabs={["Passengers", "Connecting"]}
        value={tab}
        onChange={setTab}
      />
      {tab === "Connecting" ? (
        <ConnectingPassengersPanel />
      ) : (
        <>
          <div className="flex w-full gap-3">
            {stats.map((s) => (
              <div
                key={s.label}
                className="flex flex-1 flex-col items-center gap-2 rounded-lg bg-bg-secondary px-4 py-3"
              >
                <p className={`text-2xl leading-7 font-bold ${s.color}`}>
                  {s.value}
                </p>
          ...
          <div className="flex w-full flex-col gap-2">
            ...
            <div className="flex w-full flex-col gap-1">
              {cabins.map((c) => (
                ...
                >
                  <p className="flex-1 text-sm font-medium text-fg-muted">
                    {c.label}
                  </p>
                  <p className="w-[70px] text-right text-sm font-semibold text-fg-blue">
                    {c.bkd}
                  </p>
                  <p className="w-[70px] text-right text-sm font-semibold text-fg-green">
                    {c.chk}
                  </p>
                  <p className="w-[70px] text-right text-sm font-semibold text-fg-grey-blue-chart">
                    {c.brd}
                  </p>
                </div>
              ))}
